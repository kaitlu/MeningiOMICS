...
dir.create(file.path("raw","gse77259"), recursive = TRUE)
gse77259 <- getGEO("gse77259",  # sample accession 
                   destdir = file.path("raw","gse77259"),
                   GSEMatrix = TRUE,
                   AnnotGPL = TRUE)
```
...
gse77259_cd <-
                  gse77259[["GSE77259_series_matrix.txt.gz"]] %>%           # index into list 
...
gse77259_pheno <- gse77259_cd %>%         # rename and adjust variables
                     mutate(
                            "age" = as.numeric(
                                               str_remove(age, "y") # remove the y appended to numeric
                                               ),
                            "sex" = factor(sex, 
...
gse77259_rna <-
               bind_cols(gse77259_feature["Gene symbol"], gse77259_exprs) %>% # add gene name to expression data
...
gse77259_data <- inner_join(gse77259_pheno, gse77259_rna, 
...
gse77259_info <-  
                list(
                     ...
                     accession = "GSE77259",        # dataset accession 
                     # the following are logical to determine clinical data available for dataset
                    geo_accession = "geo_accession" %in% colnames(gse77259_pheno),
                      sample_id = "sample_id" %in% colnames(gse77259_pheno),
                      age = "age" %in% colnames(gse77259_pheno),
                      sex = "sex" %in% colnames(gse77259_pheno),
                      grade = "grade" %in% colnames(gse77259_pheno),
                      histology = "histology" %in% colnames(gse77259_pheno),
                      type = "type" %in% colnames(gse77259_pheno),
                      location = "location" %in% colnames(gse77259_pheno),
                      mib1_index = "mib1_index" %in% colnames(gse77259_pheno),
                      recurrence = "recurrence" %in% colnames(gse77259_pheno),
                      recurrence_yrs = "recurrence_yrs" %in% colnames(gse77259_pheno),
                      recurrence_freq = "recurrence_freq" %in% colnames(gse77259_pheno),
                      survival_days = "survival_days" %in% colnames(gse77259_pheno),
                      survival_yrs = "survival_yrs" %in% colnames(gse77259_pheno),
                      vital_indicator = "vital_indicator" %in% colnames(gse77259_pheno),
                      disease = "disease" %in% colnames(gse77259_pheno),
                      progression = "progression" %in% colnames(gse77259_pheno),
                      radiation = "radiation" %in% colnames(gse77259_pheno),
                      rna_integrity ="rna_integrity" %in% colnames(gse77259_pheno),
                    
                    sample_numbers = nrow(gse77259_data) # number of samples    
                    )

## create dataset as list 
Schulten_H <- list(
                dataset_info = gse77259_info,             # metatag
                clinical_data = gse77259_pheno,           # clinical data
                expression_data = gse77259_rna,           # expression data
                data = gse77259_data,                     # combined, workable data
                full_GEO = gse77259                       # full original download from GEO
                )
```
```{r}
save(Schulten_H, file = "~/Documents/Thesis/mgm_eda/datasets_scripts/GSE77259_Schulten_H_KO.Rda") 
```