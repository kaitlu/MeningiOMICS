...
gse58037_pheno <-
                  gse58037[[1]] %>%           # index into list 
                  phenoData() %>%             # select phenoData
                  pData() %>%                 # apply pData
                  as_tibble() %>%             # tidy friendly
                  select("geo_accession" = geo_accession,
                         "sample_id" = `ID:ch1`,    # do we need sample name and accession number?
                         "grade" = `grade:ch1`,  # this is a mixed variable between grade and histology
                         "is_primary" = `isprimary:ch1` 
                         ) 
```
...
Clark_V <- list(
                 ...
                 data = gse58037_data)              # workable data
```
