...
gse9438_pheno <- gse9438_cd %>% 
    mutate("tissue" = "meningioma",
           ER_status = factor(ER_status, levels=c(0,1), labels = c("ER-", "ER+")),
           PR_status = factor(PR_status, levels=c(0,1), labels = c("PR-", "PR+"))
           )
```
...
```{r}
save(Claus_E, file = here("datasets/array","GSE9438_Claus_E_KO.Rda")) 
```
